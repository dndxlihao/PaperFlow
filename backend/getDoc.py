...
def _open_and_get_cookies_and_ua(headless: bool, timeout: int = 60, debug: bool = False) -> tuple[str, str]:
    ...
    try:
        ...
        # Wait longer for Cloudflare/anti-bot challenge to resolve
        time.sleep(8)
...
def download_pdf_via_selenium(
    ...
    # Record existing PDFs BEFORE download so we only pick up new ones
    existing_pdfs = set(
        ...
        while time.time() - start < timeout:
            ...
            if debug:
                if int(time.time() - start) % 5 == 0:
                    print("[DEBUG] downloading... new=", new_pdfs, "crdownload=", crds)

            if not crds and new_pdfs:
                newest = max(
...
def download_ieee_pdf_interactive(article_number: str, out_path: str, timeout: int = 120) -> bool:
    """
    Download IEEE PDF by navigating to the article page first,
    then clicking the PDF link. This better simulates real user behavior.
    """
    ...
    # Count existing PDFs before download
    existing_pdfs = set(f for f in os.listdir(download_dir) if f.lower().endswith(".pdf"))

    driver = webdriver.Chrome(options=_build_options(headless=False, download_dir=download_dir))
    stealth(driver,
            languages=["en-US", "en"],
            vendor="Google Inc.",
            platform="Win32",
            ...
            fix_hairline=True)
    try:
        # Step 1: Navigate to article page (triggers Cloudflare check)
        article_url = f"https://ieeexplore.ieee.org/document/{article_number}"
        print(f"  [IEEE] Opening article page: {article_url}")
        driver.get(article_url)
        time.sleep(10)  # Wait for Cloudflare + page load

        # Step 2: Try to navigate to PDF URL directly (now with valid session)
        pdf_url = f"https://ieeexplore.ieee.org/stampPDF/getPDF.jsp?tp=&arnumber={article_number}"
        print(f"  [IEEE] Navigating to PDF: {pdf_url}")
        driver.get(pdf_url)

        # Step 3: Wait for download
        start_t = time.time()
        while time.time() - start_t < timeout:
            current_pdfs = set(f for f in os.listdir(download_dir) if f.lower().endswith(".pdf"))
            crds = [f for f in os.listdir(download_dir) if f.lower().endswith(".crdownload")]
            ...
            if not crds and new_pdfs:
                ...
                os.replace(newest, out_path)
                print(f"  [IEEE] PDF downloaded: {out_path}")
                return True

            time.sleep(2)

        print(f"  [IEEE] Download timed out after {timeout}s")
        return False
    finally:
        driver.quit()
